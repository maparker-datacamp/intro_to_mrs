...
Welcome back to Data Management with Revolution R Enterprise. In our last
...
Throughout the last few sessions, we've occasionally seen some of RRE's
summary functions in use, like `rxGetInfo` and `rxSummary`.
In this session, we'll take a closer look at RRE's functions for calculating
...
# Create a pointer to one of RRE's sample XDFs
...
the years represented in this data? We can easily calculate group-wise means with
`rxSummary`. We just have to make a couple modifications to our code:
...
categorical variables, we typically turn to tables. RRE has two functions for 
...
Fortunately, many functions in RRE, including `rxSummary`, `rxCrossTabs`,
...
Census data that's included with RRE:
...
"per weight" that's probably my weighting variable - but to be sure, I can take a
quick look at the help file for this dataset:
...
entire group of people with the same age, sex, location, number of weeks worked, 
and approximate income, and that the "per weight" column is a count of how
many such people there are in this entire sample.
