...
entire variable down one row. In RRE, that means that we have to figure out
...
that in this session of Data Management for Revolution R Enterprise.
...
Fortunately, RRE has tools that enable chunks to communicate with each other.
Let's take a look at what a lagging function would look like in RRE.
...
value from the previous chunk. To do that, we use the special function dot rxGet,
which retrieves the value of an object called "lastValue" and an inserts that
as the first value.
...
Thankfully, the majority of functions in Revolution R Enterprise work on
