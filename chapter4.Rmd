...
So far, we've only worked with one year of mortgage default data, but RRE comes
...
success_msg("Looks good! This is a good example of combining functions from both open-source R and RRE to make your analysis simpler.")
...
RRE connects to different data sources using "data source" functions, but sometimes
...
manually can be useful for a variety of reasons:
 ...
 - Greater control over how data is imported to XDF (setting delimiters, SAS 
   import options, etc.)
 - The ability to use RRE functions directly on source data files if XDF files
   can't be used
